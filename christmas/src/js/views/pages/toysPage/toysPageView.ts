...
export class ToyPageView {
  ...
  private createHeader(
    ...
    this.rootNode.append(
      header.getTemplate(true, true, handleSearchFieldChange, handleSearchFieldClearance, this.selectedCards.length)
    );
  ...
  private createSortFilters(parentNode: HTMLElement, sortConditions: Sort, sortCards: (e: Event) => void): void {
    ...
    sortWrapper.append(
      filters.getSortingTemplate('select button', sortingNames, sortingValues, sortingChosen, sortCards)
    );
  ...
  private addButtons(parentNode: HTMLElement, handleButtonClick: (e: Event) => void): void {
    parentNode.append(
      buttons.getTemplate(['Сброс настроек', 'Сбросить фильтры'], ['resetLS', 'reset'], handleButtonClick)
    );
  }
